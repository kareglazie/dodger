...
use crate::{
    ...
    consts::{
        BUTTON_TEXT_SIZE, DARK_GREEN, FALLING_OBJECT_UPDATE_MILLIS, GRASS, GREY, LEVEL_DURATION_SECS, LIVES, WHITE, WINDOW_HEIGHT, WINDOW_WIDTH
    },
    errors::DrawError,
    ...
    utils::{
        get_level_button, half_scaling, icon_button_rect, object_scaling, player_scaling, start_point_of_button_in_set, start_point_of_centered_button, text_button_rect, text_button_rectsize, RectSize
    },
};
...
impl GameState {
    ...
    fn update_game_over(&mut self, ctx: &mut Context) -> Result<(), DrawError> {
        ...
        if is_button_clicked(
            ctx,
            text_button_rect(&self.restart_button)
        ) {
    ...
    fn update_select_level(&mut self, ctx: &mut Context) -> Result<(), DrawError> {
        ...
        for (i, _) in levels.iter().enumerate() {
            let level_button = get_level_button(i, 300.0)?;

            if is_button_clicked(
                ctx,
                text_button_rect(&level_button)
            ) {
